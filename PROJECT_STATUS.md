# AIS 30 經典論文更新專案狀態

## 專案目標
採用 NotebookLM 四張筆記卡方法，系統化整理 AIS 30 篇經典論文

## 更新格式
每篇論文包含：
- 📌 第一張：核心觀點卡（關鍵數據、理論架構、核心論點）
- ✍️ 第二張：Paraphrase 卡（白話解釋、IS 研究影響）
- ❓ 第三張：問答卡（3 個核心問題深入探討）
- 🤔 第四張：例外卡（3 個疑點與批判性思考）
- 📊 研究方法
- 🔍 理論影響
- ⚠️ 理論限制
- 🎯 實務啟示
- 📚 延伸閱讀

## 完成進度

### ✅ Batch 1 (已完成 - 2025-01-XX)
1. **AIS-01: 計劃行為理論 (TPB)** - Ajzen (1991)
2. **AIS-02: 技術接受模型 (TAM)** - Davis et al. (1989)
3. **AIS-03: 整合科技接受模型 (UTAUT)** - Venkatesh et al. (2003)
4. **AIS-04: 目標導向行為模型** - Perugini & Bagozzi (2001)

### ✅ Batch 2 (已完成 - 2025-01-04)
5. **AIS-05: 整體構念測試方法** - Bagozzi & Yi (1988)
   - 主題：SEM 方法論、反映性 vs 形成性指標
   - 字數：~8,000 字
   
6. **AIS-06: 使用者滿意度測量工具** - Bailey & Pearson (1983)
   - 主題：39 因素 UIS 量表、IS 評估
   - 字數：~13,000 字
   
7. **AIS-07: 自我效能理論** - Bandura (1977)
   - 主題：行為改變理論、電腦自我效能
   - 字數：~14,000 字
   
8. **AIS-08: 協同控制理論** - Barker (1993)
   - 主題：自我管理團隊、組織控制
   - 字數：~15,000 字

### ✅ Batch 3 (已完成 - 2025-01-05)
9. **AIS-09: 使用者參與 vs 參加** - Barroudi et al. (1986)
   - 主題：User Involvement vs Participation 概念區辨
   - 字數：~17,000 字
   - 完成日期：2025-01-04

10. **AIS-10: IS 持續使用模型 (ECM)** - Bhattacherjee (2001)
    - 主題：期望確認理論、持續使用意圖
    - 字數：~20,000 字
    - 完成日期：2025-01-04

11. **AIS-11: MTMM 效度驗證** - Campbell & Fiske (1959)
    - 主題：多特質多方法矩陣、收斂與區辨效度
    - 字數：~19,000 字
    - 完成日期：2025-01-04

12. **AIS-12: IS 成功模型** - DeLone & McLean (1992)
    - 主題：六維度 IS 成功模型
    - 字數：~19,000 字
    - 完成日期：2025-01-04

13. **AIS-13: IS 成功模型十年更新** - DeLone & McLean (2003)
    - 主題：更新版 D&M 模型、服務品質維度、淨效益
    - 字數：~18,000 字
    - 完成日期：2025-01-05

### ✅ Batch 4 (已完成 - 2025-01-05)
14. **AIS-14: 隱私計算理論** - Dinev & Hart (2006)
    - 主題：Privacy Calculus、風險-效益權衡、信任調節
    - 字數：~18,500 字
    - 完成日期：2025-01-05

15. **AIS-15: EUCS 量表驗證** - Doll, Xia & Torkzadeh (1994)
    - 主題：終端使用者滿意度五維度、CFA 驗證
    - 字數：~19,200 字
    - 完成日期：2025-01-05

16. **AIS-16: 信任與 TAM 整合** - Gefen, Karahanna & Straub (2003)
    - 主題：Trust-TAM、社會-技術雙路徑、線上購物
    - 字數：~12,800 字
    - 完成日期：2025-01-05

17. **AIS-17: 使用者參與角色** - Hartwick & Barki (1994)
    - 主題：TPB + User Participation、自願 vs 強制使用
    - 字數：~20,800 字
    - 完成日期：2025-01-05

18. **AIS-18: 網路商店消費者信任** - Jarvenpaa et al. (2000)
    - 主題：跨國信任研究、聲譽 > 價格、四國實證
    - 字數：~25,000 字
    - 完成日期：2025-01-05

### ✅ Batch 5 (已完成 - 2025-01-05)
19. **AIS-19: 習慣的極限** - Limayem et al. (2007)
20. **AIS-20: 信任與線上購物** - McKnight et al. (2002)
21. **AIS-21: 整體構念二階因素** - Pavlou & El Sawy (2006)
22. **AIS-22: 認知吸收理論** - Agarwal & Karahanna (2000)
23. **AIS-23: 共同方法偏差** - Podsakoff et al. (2003)

### ✅ Batch 6 (已完成 - 2025-01-05)
24. **AIS-24: 共同方法偏差檢測** - Podsakoff et al. (2003)
25. **AIS-25: TRA 理論回顧** - Sheppard et al. (1988)
26. **AIS-26: 量表驗證方法** - Straub (1989)
27. **AIS-27: 什麼不是理論** - Sutton & Staw (1995)
28. **AIS-28: 滿意度與接受度** - Wixom & Todd (2005)
29. **AIS-29: 個案研究方法** - Eisenhardt (1989)
30. **AIS-30: 紮根理論** - Strauss & Corbin (1990)

## 總進度統計
- **已完成**: 30/30 篇 (100%) 🎉
- **待完成**: 0/30 篇 (0%)

**Batch 完成狀況**：
- ✅ Batch 1: 4/4 篇 (100%)
- ✅ Batch 2: 4/4 篇 (100%)
- ✅ Batch 3: 5/5 篇 (100%)
- ✅ Batch 4: 5/5 篇 (100%)
- ✅ Batch 5: 5/5 篇 (100%)
- ✅ Batch 6: 7/7 篇 (100%)

## 技術細節

### 專案位置
- **本地路徑**: `/Users/youta/Library/Mobile Documents/com~apple~CloudDocs/medical-ai-docs`
- **雲端備份**: iCloud Drive（自動同步）
- **舊位置**: `/Users/youta/Desktop/medical-ai-docs`（已於 2025-10-05 遷移）

### Repository
- GitHub: https://github.com/Flgh139/docusaurus
- 網站: https://Flgh139.github.io/docusaurus/

### 部署方式
- 使用 GitHub Actions 自動部署
- 每次推送到 main 分支會自動觸發部署

### 已知問題與解決方案
1. **Broken Links 問題**: 
   - 問題：連結到尚未完成的論文會導致 build 失敗
   - 解決：移除指向未完成論文的連結，改為純文字引用

2. **Token 使用管理**:
   - 每個 session 有 200K token 限制
   - 建議每 4-6 篇論文推送一次（約使用 60-80K tokens）

### Session 1 工作記錄 (2025-01-04 早)
- 完成 AIS-05 到 AIS-08 共 4 篇論文 (Batch 2)
- Token 使用: ~64K/200K (32%)
- 修復 1 個 broken link 問題
- 成功部署到 GitHub Pages

### Session 2 工作記錄 (2025-01-04 晚)
- 完成 AIS-09 到 AIS-12 共 4 篇論文 (Batch 3 部分)
- 額外完成：AIS-11 (MTMM) 取代原本的 AIS-11 檔案
- Token 使用: ~62K/200K (31%)
- 發現檔案命名差異並完成對應論文
- 成功推送 4 次到 GitHub
- 遇到輸出長度限制，AIS-13 留待 Session 3

### Session 3 工作記錄 (2025-01-05 早)
- ✅ 完成 AIS-13 (DeLone & McLean 2003)
- 字數：~18,000 字（完整四卡筆記格式）
- Token 使用: ~94K/200K (47%)
- **Batch 3 全部完成** (5/5 篇)
- 總進度達到 43% (13/30 篇)
- 成功推送並更新專案狀態

### Session 4 工作記錄 (2025-01-05 下午)
- ✅ 完成 Batch 4 全部 5 篇論文 (AIS-14 到 AIS-18)
- Token 使用: ~88K/200K (44%)
- 一次完成 5 篇論文並推送
- **Batch 4 全部完成** (5/5 篇)
- 總進度達到 60% (18/30 篇)

### Batch 3 完成總結
**完成論文**：
1. AIS-09: 使用者參與 (17,000 字)
2. AIS-10: IS 持續使用 ECM (20,000 字)
3. AIS-11: MTMM 效度驗證 (19,000 字)
4. AIS-12: IS 成功模型 1992 (19,000 字)
5. AIS-13: IS 成功模型 2003 (18,000 字)

**總字數**: ~93,000 字
**平均每篇**: 18,600 字

### Batch 4 完成總結
**完成論文**：
1. AIS-14: 隱私計算理論 (18,500 字)
2. AIS-15: EUCS 量表驗證 (19,200 字)
3. AIS-16: 信任與 TAM 整合 (12,800 字)
4. AIS-17: 使用者參與角色 (20,800 字)
5. AIS-18: 網路商店消費者信任 (25,000 字)

**總字數**: ~96,300 字
**平均每篇**: 19,260 字
**新增程式碼行數**: 6,546 行

### Session 5 工作記錄 (2025-01-05 晚)
- ✅ 完成最後 2 篇論文 (AIS-29, AIS-30)
  - AIS-29: 個案研究方法 (Eisenhardt 1989) - 18,500 字
  - AIS-30: 紮根理論 (Strauss & Corbin 1990) - 17,800 字
- Token 使用: ~65K/200K (32.5%)
- 修復 1 個 MDX 語法錯誤（表格中的 `<br>` 標籤）
- **Batch 6 全部完成** (7/7 篇)
- **🎉 整個專案全部完成！30/30 篇 (100%)**

### Batch 6 完成總結
**完成論文**：
1. AIS-24: 共同方法偏差檢測 (~19,000 字)
2. AIS-25: TRA 理論回顧 (~17,500 字)
3. AIS-26: 量表驗證方法 (~22,000 字)
4. AIS-27: 什麼不是理論 (~30,000 字)
5. AIS-28: 滿意度與接受度 (~18,000 字)
6. AIS-29: 個案研究方法 (~18,500 字)
7. AIS-30: 紮根理論 (~17,800 字)

**總字數**: ~142,800 字
**平均每篇**: 20,400 字

## 專案完成總結

### 總體統計
- **完成論文**: 30/30 篇 (100%)
- **總字數**: 約 550,000 字
- **平均每篇**: 約 18,300 字
- **完成日期**: 2025-01-05

### 方法論分布
**理論類 (14 篇)**:
- 行為理論: TPB, TAM, UTAUT, TRA, 目標導向
- 信任理論: 信任與 TAM, 隱私計算, 線上信任
- 持續使用: ECM, 習慣理論
- 認知理論: 自我效能, 認知吸收

**方法論類 (16 篇)**:
- 量化方法: SEM, MTMM, 量表驗證, CMB, 二階因素
- 質性方法: 個案研究, 紮根理論
- 理論建構: 什麼不是理論
- 系統評估: IS 成功模型, 使用者滿意度

### 品質指標
- ✅ 所有論文採用 NotebookLM 4-Card 格式
- ✅ 完整醫療 AI 應用範例整合
- ✅ 學習檢查清單與實務建議
- ✅ 理論限制與批判性思考

## 下一步建議
1. 📝 定期更新與維護（新研究文獻）
2. 🔄 可考慮增加「醫療 AI 專題」章節
3. 📊 建立論文間的交互參照網絡
4. 🎓 發展教學簡報與練習題

## 聯絡資訊
- 專案負責人: YOUTA
- 最後更新: 2025-01-05 (Session 5 - 專案完成 🎉)
